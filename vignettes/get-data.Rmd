...
to the HRSL. Otherwise, we don't modify values.
```{r hrsl_original}
...
aggregate_and_write <- function(raster, name) {
  aggregated <- raster::aggregate(raster, face = 10, fun = sum, expand = TRUE)
  popbioko::write_aligned_raster(
    aggregated,
...
bimep_on_landscan <- bimep_on_grid(landscan, bioko_sf, local_directory = data_dir)
popbioko::write_aligned_raster(
  landscan,
...
# Align to Grids from a BIMEP 100m Grid
This section uses BIMEP data at a 100m grid level.
