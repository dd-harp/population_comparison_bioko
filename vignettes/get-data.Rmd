...
The output is all GeoTIFFs.

- `hrsl_bimep.tif`, with HRSL and BIMEP on the 1 km grid that BIMEP
  defined.
- `landscan_bimep.tif`, with the BIMEP data on LandScan's 1 km grid.
- `landscan_crop.tif`, with the LandScan data, cropped to Bioko's
  bounds. This is in a separate GeoTIFF because the data types are integer and float.
- `worldpop_bimep.tif`, with two layers, one for WorldPop and one for
  BIMEP on the WorldPop grid, cropped to the Bioko boundary.
  This is on a 1 km grid.
- `hrsl_crop.tif`, which is HRSL cropped to the Bioko boundary,
  This is on a 100 m grid.
- `worldpop_crop.tif`, which is WorldPop cropped to the Bioko boundary,
  on a 100 m grid.
...
raster::writeRaster(
  both_layers,
  filename = fs::path(data_dir, "hrsl_bimep"),
  format = "GTiff",
  overwrite = TRUE
  )
names(hrsl_raster_crop) <- "HRSL"
raster::writeRaster(
  hrsl_raster_crop,
  filename = fs::path(data_dir, "hrsl_crop"),
  format = "GTiff",
  overwrite = TRUE
  )
```
...
raster::writeRaster(
  bimep_landscan_raster,
  filename = fs::path(data_dir, "landscan_bimep"),
  format = "GTiff",
  overwrite = TRUE
  )
raster::writeRaster(
  landscan,
  filename = fs::path(data_dir, "landscan_crop"),
  format = "GTiff",
  overwrite = TRUE
  )
```
...
raster::writeRaster(
  wp_layers,
  filename = fs::path(data_dir, "worldpop_bimep"),
  format = "GTiff",
  overwrite = TRUE
  )
# Save the 100m WorldPop, cropped to Bioko.
names(worldpop) <- "WorldPop"
raster::writeRaster(
  worldpop,
  filename = fs::path(data_dir, "worldpop_crop"),
  format = "GTiff",
  overwrite = TRUE
  )
```
