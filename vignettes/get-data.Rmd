...
aggregate_and_write <- function(raster, name) {
  ...
  cat(paste("aggregating factor for", name, "is", fact, "\n"))
...
```{r worldpop_to_grid}
wp_options <- list(raw = "GNQ15v2.tif", un_adjusted = "GNQ15adjv2.tif")
worldpop <- raster::raster(fs::path(
  data_dir, "Equatorial_Guinea_100m_Population", wp_options[["un_adjusted"]]))
